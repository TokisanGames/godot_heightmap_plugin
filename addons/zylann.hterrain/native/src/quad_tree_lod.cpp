...
unsigned int QuadTreeLod::_allocate_children() {
    if (_free_indices.size() == 0) {
        unsigned int i0 = static_cast<unsigned int>(_quad_pool.size());
        _quad_pool.resize(i0 + 4);
        return i0;
    } else {
...
void QuadTreeLod::create_from_sizes(int base_size, int full_size) {
    ...
    _quad_pool.reserve(node_count);                               // e.g. (4^6 / 3 ) - 1 = 1364(.33) excluding root
